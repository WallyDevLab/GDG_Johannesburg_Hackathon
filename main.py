...
genai.configure(api_key="AIzaSyD1FL0jSWFLQBP4tplGslox4TXXdNtCA8g")
...
prompt_parts = [
  "you are a python developer. you need to create an application that tracks stock inventory and provides stocking suggestions based on customer interactions and spending through machine learning and also has the functionality to scan barcodes and tracking the expiry date, then convert it to a Qr code which a merchant can use to track inventory and expiry date. Merchants should be able to source stock from suppliers and track their own inventory.\nSupply a Jason file, use JavaScipt to connect the MySql database and use firebase for security and logging in. \nAlso supply an attractive front end",
]
